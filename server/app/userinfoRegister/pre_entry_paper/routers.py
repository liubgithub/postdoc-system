...
def create_paper(paper: schemas.PreEntryPaperCreate, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    db_paper = models.PreEntryPaper(**paper.dict(exclude={"user_id", "achievement_id"}), user_id=current_user.id)
    db.add(db_paper)
    db.commit()
    db.refresh(db_paper)
    return db_paper
...
def update_paper(id: int, paper: schemas.PreEntryPaperUpdate, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    if not db_paper:
        raise HTTPException(status_code=404, detail="Paper not found")
    for key, value in paper.dict(exclude_unset=True, exclude={"user_id", "achievement_id"}).items():
