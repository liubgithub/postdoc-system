...
import Achievement_1 from '@/pages/OutWorkstation/achres_1';
import Assessment from '@/pages/OutWorkstation/assessment';
...
export default defineComponent({
  ...
  setup() {
    ...
    // 出站考核数据
    const outAssessmentData = ref<any>(null);
    const loading = ref(false);

    // 获取学生出站考核数据
    const fetchStudentOutAssessmentData = async () => {
      ...
      try {
        loading.value = true;
        const response = await getStudentDetail(parseInt(studentInfo.value.userId), "出站考核");
        
        if (response.data) {
          outAssessmentData.value = response.data;
          console.log("获取的出站考核数据:", response.data);
          ...
          console.error("获取出站考核数据失败:", response.error);
          ElMessage.warning("未找到对应的出站考核数据");
        }
      } catch (error) {
        console.error("获取出站考核数据失败:", error);
        ElMessage.warning("获取出站考核数据失败");
      } finally {
        loading.value = false;
      }
    };
    ...
    // 页面加载时获取学生数据
    onMounted(() => {
      fetchStudentOutAssessmentData();
    });

    // 返回按钮处理
    const handleBack = () => {
      router.push('/teacher/outCheck');
    };
    ...
    const handleApprove = async () => {
      ...
      try {
        ...
        if (response.data) {
          ...
          // 重新获取数据以更新状态
          await fetchStudentOutAssessmentData();
        } else if (response.error) {
              ...
              <div style={{ flex: 1, overflowY: "auto" }}>
                ...
                {loading.value ? (
                  ...
                  <div>
                    ...
                    <Assessment showOutCheckButtons={false} />
                  </div>
                )}
              </div>
