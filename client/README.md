...
- "TSX比模板性能弱"这种说法是指TSX组件本身会全量刷新，但并不引起子组件的刷新，绝大部分情况下我们并不需要这种1ms都不到的粒度的优化，如果真的需要，可以通过拆分大组件为多个小组件或使用Computed实现，日常开发还是以效率为重，TSX写法更灵活，开发效率更高。
- TSX不需要声明使用了哪些组件，直接import即可。
- 除TSX完全无法实现的极端情况外（非常罕见），都写TSX组件，独立页面有必要的话也可以写SFC，总的来说，项目的大部分代码尽量不依赖当前的Vue插件（TSX语言由VSCode本身完全支持）。
- 直接用props传递组件、函数、JSX节点，基本不需要定义emits、slots（也尽量避免定义），写起来更方便。
#### 页面级Store（注入）
- 每个页面都通过Provide/Inject实现页面级Store。
- 通过```export default Symbol() as InjectionKey<T>```来定义Key，方便使用且可以获得自动补全与类型检查。
- 避免了绝大部分定义和传递Props，同时请尽量将页面拆分为独立的子组件，提高可维护性。
- 在必要时可以直接将页面级状态提升到全局Pinia中，修改代码不多。
#### Vanilla Extract
- 局部CSS方案
